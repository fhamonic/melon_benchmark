...
private:
    ...
    void __push_heap(Difference holeIndex, Difference topIndex,
                     Pair && p) {
        Difference parent = (holeIndex - 1) / 2;
        while(holeIndex > topIndex && cmp(p.second, (heap_array.begin() + parent)->second)) {
            __heap_move(holeIndex, std::move(*(heap_array.begin() + parent)));
            holeIndex = parent;
...
public:
    void push(Pair && p) {
        ...
        heap_array.resize(n + 1);
        // bubbleUp(n, p);
        __push_heap(Difference(n), Difference(0),
                    std::move(p));
    }
    void push(const Node i, const Prio p) { push(Pair(i, p)); }
    bool contains(const Node u) const { return indices_map[u] > 0; }
