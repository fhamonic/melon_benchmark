...
data = [
    [int(float(row['time_ms']))
     for row in readCSV('results/dijkstra/snap/bgl_adjacency_list_vecS.csv')],
    [int(float(row['time_ms']))
...
ax.set_title(
    'Dijkstra average runtime on large directed unweighted graphs from SNAP datasets')
ax.set_xticks(x)
...
plt.savefig('results/dijkstra/snap/plot.png')
