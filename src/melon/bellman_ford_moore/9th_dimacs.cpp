...
struct bench_bellman_ford_moore_traits {
...
template <typename _Graph, typename _Value>
struct BM {
    using traits = bench_bellman_ford_moore_traits<_Graph, _Value>;
    ...
    static std::string result_checksum(
        ...
        checksum cs;
        for(auto && s : sources) {
            auto algo = bellman_ford_moore(traits{}, graph, length_map, s);
            algo.run();
